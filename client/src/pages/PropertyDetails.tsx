...
export default function PropertyDetails() {
  ...
  // Atualizar título do documento/header quando propriedade carregar
  useEffect(() => {
    if (property) {
      const ownerName = property.owners && property.owners.length > 0 
        ? property.owners[0].fullName 
        : 'Sem proprietário';
      const pageTitle = `${getSequenceNumber()} - ${ownerName} - ${property.street}, ${property.number}`;
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <PageLoader
          size="lg"
          message="Carregando detalhes do imóvel..."
        />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 card-desktop-compact">
      <div className="w-full">
        {/* Header Section with Property Info and Navigation */}
        <div className="bg-white shadow-sm">
          <div className="px-6 py-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-xl font-semibold text-gray-900">
                    {getSequenceNumber()} - {property.owners && property.owners.length > 0 
                      ? property.owners[0].fullName 
                      : 'Sem proprietário'} - {property.street}, {property.number}
                  </h1>
            ...
            {/* Progress Section - Now as Banner */}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-3">
                  {stages.map((stage, index) => {
                    ...
                    return (
                      <div
                        key={stage.id}
                        className={`text-center p-3 rounded-lg border-2 transition-all ${stageStatus === 'completed'
                      ...
                      >
                        <div className={`w-10 h-10 mx-auto mb-2 rounded-full flex items-center justify-center ${stageStatus === 'completed'
                        ...
                        <h3 className={`text-xs font-medium mb-1 ${stageStatus !== 'pending' ? 'text-gray-900' : 'text-gray-500'
                          }`}>
                          {stage.label}
                        </h3>
                    ...
                    <div>
                      ...
                      <div className="flex items-start space-x-2">
                        ...
                        <div>
                          ...
                          {property.cep && (
                            <p className="text-sm text-gray-600">CEP: {property.cep}</p>
                          )}
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {property.registrationNumber && (
                            <div>
                              ...
                              <p className="text-sm text-gray-900">{property.registrationNumber}</p>
                            </div>
                          )}
                          {property.municipalRegistration && (
                            <div>
                              ...
                              <p className="text-sm text-gray-900">{property.municipalRegistration}</p>
                            </div>
                          )}
                        </div>
                      </>
                  ...
                  <CardContent>
                    {property.owners && property.owners.length > 0 ? (
                      <div className="space-y-4">
                        {property.owners.map((owner: any, index: number) => (
                          <div key={index} className="flex items-start space-x-4 p-4 border border-gray-200 rounded-lg">
                            <div className="flex-1 space-y-2">
                              <h4 className="text-sm font-medium text-gray-900">{owner.fullName}</h4>
                  ...
                  <CardContent>
                    {documentsLoading ? (
                      <div className="flex items-center justify-center py-8">
                        ...
                        <span className="ml-3 text-sm text-gray-600">Carregando documentos...</span>
                      </div>
                        ...
                        {documents.map((doc: any, index: number) => (
                          <div key={index} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg bg-green-50">
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <span className="text-xs font-medium text-green-700">{doc.name}</span>
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm">Nenhum documento foi enviado ainda</p>
                        <Button
                          variant="outline"
                          className="mt-4"
                          onClick={() => setShowPropertyModal(true)}
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <p className="text-xs font-medium">Imóvel cadastrado</p>
                ...
                <motion.div variants={getListItemVariants()}>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-gray-600">Dias no processo</span>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-gray-600">Etapa atual</span>
                      <span className="text-xs font-medium">{property.currentStage || 1}/8</span>
                    </div>
                    <div className="flex justify-between items-center">
