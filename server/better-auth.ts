...
export const auth = betterAuth({
  database: drizzleAdapter(db, {
    provider: "pg",
    // Since our existing users table structure is different, 
    // we'll let Better Auth create its own tables
  }),
  emailAndPassword: {
    enabled: true,
  ...
  trustedOrigins: ["http://localhost:5000", "http://localhost:3000", "http://localhost:5173"],
});
